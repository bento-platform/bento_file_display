    ...
    <>
      ...
      <Collapse
        ...
        items={entries.map(([key, value]) => ({
          key,
          label: <code className="bento-json-display__code">{key}</code>,
...
const JsonDisplayInner = ({ jsonSrc, showObjectWithReactJson, showArrayTitle }: JsonDisplayProps) => {
  ...
  if (jsonSrc === null) {
    return <code className="bento-json-display__code">null</code>;
  }
  ...
  // Display primitive
  return <code className="bento-json-display__code">{JSON.stringify(jsonSrc)}</code>;
};

const JsonDisplay = (props: JsonDisplayProps) => (
  <div className="bento-json-display">
    <JsonDisplayInner {...props} />
  </div>
);
